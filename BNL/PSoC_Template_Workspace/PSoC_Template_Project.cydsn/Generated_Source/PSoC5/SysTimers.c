...
void    SysTimers_Start(void)
{
	...
	if( SysTimers_initVar == 0 )
	{
		...
		{
		    SysTimers_TimerSCount[index] = SysTimers_INVALID_PERIOD;
			SysTimers_TimerPeriod[index] = SysTimers_INVALID_PERIOD;
			SysTimers_TimerStatus[index] = SysTimers_NOT_IN_USE;	
		}
...
uint32  SysTimers_GetTimer(uint32 timerValue )
{
	...
	/* Search for a unused timer */	for(index=0; index < SysTimers_TIMERS; index++)
	{
		if(!(SysTimers_TimerStatus[index] & SysTimers_ACTIVE))
		{
		    #if(SysTimers_TIMER_MODE == SysTimers_MODE_FAST_IRQ)
			...
			SysTimers_TimerStatus[index] = SysTimers_IN_USE | SysTimers_ACTIVE;
			timerID = index + 1;
			break;
		}
	}
...
void  SysTimers_ReleaseAllTimers(void)
{
	...
	{
	    SysTimers_TimerSCount[index]--;
		if(SysTimers_TimerStatus[index] == 0)
		{
