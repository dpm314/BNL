...
I2CM1_BACKUP_STRUCT I2CM1_backup =
{
    I2CM1_DISABLE,
    ...
    I2CM1_DEFAULT_CFG,
    I2CM1_DEFAULT_ADDR,
    LO8(I2CM1_DEFAULT_DIVIDE_FACTOR),
    HI8(I2CM1_DEFAULT_DIVIDE_FACTOR),
#else  /* (I2CM1_UDB_IMPLEMENTED) */
    I2CM1_DEFAULT_CFG,
#endif /* (I2CM1_FF_IMPLEMENTED) */

#if (I2CM1_TIMEOUT_ENABLED)
    I2CM1_DEFAULT_TMOUT_PERIOD,
    I2CM1_DEFAULT_TMOUT_INTR_MASK,
#endif /* (I2CM1_TIMEOUT_ENABLED) */
};

#if ((I2CM1_FF_IMPLEMENTED) && (I2CM1_WAKEUP_ENABLED))
    volatile uint8 I2CM1_wakeupSource;
#endif /* ((I2CM1_FF_IMPLEMENTED) && (I2CM1_WAKEUP_ENABLED)) */


/*******************************************************************************
* Function Name: I2CM1_SaveConfig
********************************************************************************
*
* Summary:
...
*
* Global Variables:
*  I2CM1_backup - The global variable used to save the component
...
{
#if (I2CM1_FF_IMPLEMENTED)
    #if (I2CM1_WAKEUP_ENABLED)
        uint8 intState;
    #endif /* (I2CM1_WAKEUP_ENABLED) */

    /* Store registers before enter low power mode */
    I2CM1_backup.cfg     = I2CM1_CFG_REG;
    I2CM1_backup.xcfg    = I2CM1_XCFG_REG;
...
#if (I2CM1_WAKEUP_ENABLED)
    /* Disable master */
    I2CM1_CFG_REG &= (uint8) ~I2CM1_ENABLE_MASTER;

    /* Enable backup regulator to keep block powered in low power mode */
    intState = CyEnterCriticalSection();
    I2CM1_PWRSYS_CR1_REG |= I2CM1_PWRSYS_CR1_I2C_REG_BACKUP;
    ...
    */
    ...
    I2CM1_XCFG_REG |= I2CM1_XCFG_FORCE_NACK;
    while (0u == (I2CM1_XCFG_REG & I2CM1_XCFG_RDY_TO_SLEEP))
    {
    }

    /* Setup wakeup interrupt */
    I2CM1_DisableInt();
    (void) CyIntSetVector(I2CM1_ISR_NUMBER, &I2CM1_WAKEUP_ISR);
    I2CM1_wakeupSource = 0u;
    I2CM1_EnableInt();
#endif /* (I2CM1_WAKEUP_ENABLED) */

#else
    /* Store only address match bit */
    I2CM1_backup.control = (I2CM1_CFG_REG & I2CM1_CTRL_ANY_ADDRESS_MASK);
#endif /* (I2CM1_FF_IMPLEMENTED) */

#if (I2CM1_TIMEOUT_ENABLED)
    I2CM1_TimeoutSaveConfig();
#endif /* (I2CM1_TIMEOUT_ENABLED) */
}


/*******************************************************************************
* Function Name: I2CM1_Sleep
********************************************************************************
*
* Summary:
...
void I2CM1_Sleep(void) 
{
    ...
    I2CM1_backup.enableState = I2CM1_DISABLE;
...
#else
    /* Store enable state */
    I2CM1_backup.enableState = (uint8) I2CM1_IS_ENABLED;

    if (0u != I2CM1_backup.enableState)
    {
        I2CM1_Stop();
    }
#endif /* (I2CM1_WAKEUP_ENABLED) */
...
/*******************************************************************************
* Function Name: I2CM1_RestoreConfig
********************************************************************************
*
* Summary:
*  The Enable wakeup from Sleep Mode selection influences this function
...
*
* Side Effects:
*  Calling this function before I2CM1_SaveConfig() or
*  I2CM1_Sleep() will lead to unpredictable results.
*
*******************************************************************************/
void I2CM1_RestoreConfig(void) 
{
    ...
    if (I2CM1_CHECK_PWRSYS_I2C_BACKUP)
    /* Low power mode was Sleep - backup regulator is enabled */
    {
        /* Enable backup regulator in active mode */
        intState = CyEnterCriticalSection();
        I2CM1_PWRSYS_CR1_REG &= (uint8) ~I2CM1_PWRSYS_CR1_I2C_REG_BACKUP;
        CyExitCriticalSection(intState);

        /* Restore master */
        I2CM1_CFG_REG = I2CM1_backup.cfg;
    }
    else
    /* Low power mode was Hibernate - backup regulator is disabled. All registers are cleared */
    {
        ...
        I2CM1_backup.enableState = I2CM1_ENABLE;
    #endif /* (I2CM1_WAKEUP_ENABLED) */

        /* Restore component registers after Hibernate */
        I2CM1_XCFG_REG    = I2CM1_backup.xcfg;
        I2CM1_CFG_REG     = I2CM1_backup.cfg;
...
#else
    I2CM1_CFG_REG = I2CM1_backup.control;
#endif /* (I2CM1_FF_IMPLEMENTED) */

#if (I2CM1_TIMEOUT_ENABLED)
    I2CM1_TimeoutRestoreConfig();
#endif /* (I2CM1_TIMEOUT_ENABLED) */
}


/*******************************************************************************
* Function Name: I2CM1_Wakeup
********************************************************************************
*
* Summary:
